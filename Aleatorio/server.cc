...
zframe_t* getWorkerFor(string operation) {  //zframe_t* getWorkerFor(string operation) {
  // Assumes there are at least one worker registered.

  /*Aleatorio*/
  int len=wr[operation].size();
  int random= rand() % len; 
  //cout<<"Len: "<<len<<"Random: "<<random<<endl;  //Comprobacion Random
